  ...
  async ({ step, event }) => {
    ...
    const events = userNotifications.map((notification) => {
      return {
        ...
        data: {
          aiPropmp: notification.aiPropmp ?? undefined,
...
export const sendDailyUserJobListingEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const { jobListings, aiPropmp } = event.data;
    const user = event.user;

    if (jobListings.length === 0) return null;

    let matchingJobListings: typeof jobListings = [];
    if (aiPropmp == null || aiPropmp.trim() === "") {
      matchingJobListings = jobListings;
    } else {
      const matchingIds = await getMatchingJobListings(aiPropmp, jobListings);
    ...
    await step.run("send-email", async () => {
      await resend.emails.send({
        from: "Intelli-Job <onboarding@resend.dev>",
        to: user.email,
...
export const prepareDailyOrganizationUserApplicationNotifications = inngest.createFunction(
  ...
  async ({ step, event }) => {
    ...
    const events = Object.entries(groupedNotifications).map(([,settings]) => {
      ...
      const filteredApplciations = applications.filter(a => {
      ...
      if (filteredApplciations.length === 0) return null
      ...
      return {
        ...
        data: { applications: filteredApplciations }
      } as const satisfies GetEvents<
        typeof inngest
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-email", async () => {
      await resend.emails.send({
        from: "Intelli-Job <onboarding@resend.dev>",
        to: user.email,
        subject: "Daily job listing applciations",
